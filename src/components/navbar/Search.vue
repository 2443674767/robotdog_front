<template>
  <div class="search-trigger" @click="visible = true">
    <icon-search :size="18" style="margin-right: 4px;color:var(--color-neutral-6);" />
    ...
    >
      <template #title>
        <div class="search-input-wrapper">
          <icon-search :size="24" />
          <input ref="searchInput" v-model="searchKeyword" :placeholder="$t('navbar.seach.placeholder')" class="search-input">
          <div class="esc-tip">
            {{ $t('navbar.seach.esc') }}
          </div>
        </div>
      ...
      <div class="search-content">
        <div v-if="searchResults.length">
          <div class="result-count">
             {{ $t('navbar.seach.results', { resultCount:searchResults.length } ) }}
          </div>
        ...
        <div v-if="searchHistory.length==0&&searchResults.length==0" class="empty-wrap">{{searchKeyword?$t('navbar.seach.tig.empty',{searchKeyword}):$t('navbar.seach.tig')}}</div>
      </div>
      <template #footer>
        <div class="shortcut">
          <Icon icon="svgfont-select" :size="22" class="shortcut-icon" />
          <div>{{ $t('navbar.seach.switch') }}</div>
        </div>
        <div class="shortcut">
          <Icon icon="svgfont-enter" :size="22" class="shortcut-icon" />
          <div>{{ $t('navbar.seach.select') }}</div>
        </div>
      </template>
    </a-modal>
  </div>
