...
class Scrollbar extends EvemtEmitter3 {
	...
	enableScrollval: boolean = true;
	scrollval?: ScrollbarCustomeOptions;
	isScrolling = false;
	/**
	...
	getWidth() {
		...
		const offsetWidth = this.scrollval?.getOffsetWidth
			? this.scrollval.getOffsetWidth(width)
			: width;
		return offsetWidth;
	}
		...
		if (element) {
			...
			if (
				...
				let left =
					...
					this.content!.width();
				if (this.scrollval) {
					const { onScrollX, getScrollLeft } = this.scrollval;
			...
			const left = this.scrollval?.getScrollLeft
				? this.scrollval.getScrollLeft(element.scrollLeft)
				: element.scrollLeft;
			if (this.scrollval) {
				const { onScrollX } = this.scrollval;
				if (onScrollX) {
					this.isScrolling = true;
					const result = onScrollX(left);
					if (result > 0) element.scrollLeft = result;
	 ...
	 */
	enableScroll() {
		this.enableScrollval = true;
	}
	/**
	 * 禁用鼠标在内容节点上滚动或在移动设备使用手指滑动
	 */
	disableScroll() {
		this.enableScrollval = false;
	}

	scroll = (event: Event | { top?: number; left?: number }) => {
		let top = 0;
		let left = 0;
		if (!this.scrollval && event instanceof Event) {
	...
	wheelXScroll = throttle(
		(event: any) => {
			...
			const width = this.container.width();
			const containerWidth = this.scrollval?.getOffsetWidth
				? this.scrollval.getOffsetWidth(width)
				: width;
			const step = Math.max(
				containerWidth /
			...
			let left =
				(this.scrollval?.getScrollLeft
					? this.scrollval.getScrollLeft(containerElement.scrollLeft)
	...
	bindWheelScroll = (event: any) => {
		if (!this.enableScrollval) return;
	...
	bindContainerTouchX = (event: TouchEvent) => {
		if (!event.target || !this.enableScrollval) return;
	...
	reRenderShadow = (width: number) => {
		if (this.shadow) {
			...
			if (element) {
				this.shadowLeft?.css(
					'left',
					(this.scrollval ? element.scrollLeft : width) + 'px',
				);
			}
