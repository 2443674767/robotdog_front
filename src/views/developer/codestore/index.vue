<template>
  ...
  <script lang="ts" setup>
    ...
    //api
    import { getList,asyncVersion,upBaseCode,DataItem,getCodeCate,upPrivateHouse,checkIsPay,getInstallPack} from '@/api/developer/codestore';
    import { downCode,installCode,uninstallCode,installLocalCode } from '@/api/developer/packinstall';
    //表单
    ...
    //配置仓库地址
    // let baseRootUrl = import.meta.env.VITE_APP_ENV=="production"?"https://api.goflys.cn":"http://localhost:8110"//社区本地开发使用
    let baseRootUrl = "https://api.goflys.cn"
    const confhouse= ref<any>({
      public:{
          url:baseRootUrl,
      },
      //私有地址
   ...
   const getCateList=async()=>{
    ...
    try {
        ...
        if(resdata){
          ...
          confhouse.value.private.url=resdata.privateHouse
          //初始化数据
          renderData.value=[]
      ...
      const controller = new AbortController();
        (async function requestWrap() {
          ...
          try {
            ...
            onError(error);
             Message.loading({content:"上传本地代码中",id:"upkey",duration:1})
          }
        })();
        return {
          abort() {
   ...
   const search = () => {
    ...
    const handleAsyncVersion=async()=>{
      try {
         ...
         if(res){
           Message.loading({content:"检查版本更新中",id:"asyncVersion",duration:1})
           if(res.type==0){
            Modal.warning({
              title: res.title ,
              content:res.content,
            ...
            Modal.success({
              ...
              onOk:async()=>{
                ...
                try {
                  ...
                  const vs= await upBaseCode({code_token:formModel.value.code_token},baseRootUrl);
                  if (vs){
                    version.value=vs
                  }
                  Message.success({content:"更新成功",id:"asyncVersion",duration:1000})
                }catch (error) {
                  Message.success({content:"更新中",id:"asyncVersion",duration:1})
                } 
              },
            });
           }
         } else{
          Message.success({content:"检查版本更新中",id:"asyncVersion",duration:1})
         }
      }catch (error) {
        Message.success({content:"检查版本更新中",id:"asyncVersion",duration:1})
      } 
  ...
  //获取是否安装状态
   ...
   const upprivateURl=async()=>{
    try {
        ...
        await upPrivateHouse({privateHouse: confhouse.value.private.url});
