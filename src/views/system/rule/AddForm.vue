    ...
    <a-form ref="formRef" :model="formData" auto-label-width>
     <a-form-item field="type" label="菜单类型" style="margin-bottom:15px;">
      ...
      <a-row :gutter="16">
        ...
        <a-col :span="12"  v-if="formData.type!=2">
          <a-form-item field="routepath" label="路由地址" validate-trigger="input" :help="formData.type==1?'如果路由地址不需要继承上层地址，可在名称前添加/，如：/user':''" :rules="[{required:true,message:'请填写路由地址'}]" tooltip="前端访问url路径，如：https://goflys.cn/detail中的detail，建议和路由名称或者文件目录一致">
            <a-input-group style="width: 100%;">
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
          ...
          updateRoutepath()//根据菜单类型和菜单层级改变路由地址
      });
      //根据菜单类型和菜单层级改变路由地址
      const updateRoutepath=()=>{
          if(formData.value.type==1&&formData.value.pid!=0){
     ...
     const handleSubmit = async () => {
      ...
      const handleChekHaseRule=(value:any, cb:any)=>{
        return new Promise(async(resolve) => {
            if(value){
              const resData = await checkHaseRule({id:formData.value.id,routename:value,codelocation:"admin_auth_rule"});
              if(!resData){
                cb("该路由已被占用请换一个")
              }
            }
