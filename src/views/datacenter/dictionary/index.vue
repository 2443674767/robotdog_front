      ...
      <div class="flex-page content">
        ...
        <div class="table-toolbar flex flex-between">
          ...
          <tabletool :showbtn="['create','refresh','selectdensity','fullscreen']"
           @create="createData" @refresh="fetchData" @selectdensity="(data:any)=>size=data" @fullscreen="(data:any)=>isFullscreen=data"></tabletool>
          </div>
        </div>
        <div class="table-container">
          ...
          >
            ...
            <template #operations="{ record }">
              <template v-if="record.data_from=='admin'">
...
<script lang="ts" setup>
  ...
  import { getmenuList,delMenuList, menuItem} from '@/api/datacenter/dicgroup';
  ...
  const handleClickMenu=(Dicgroup:any)=>{
    renderData.value =[]
    pagination.current =1
    setLoading(true);
    selectedKeys.value=[Dicgroup.id]
    handleTablename.value=Dicgroup.tablename
    handleGroupId.value=Dicgroup.id
    Menuitem.value=Dicgroup
    fetchData();
  }
  //添加菜单数据
       ...
       Modal.warning({
        ...
        onOk:(async()=>{
            try {
              ...
              await delMenuList({id:selectedKeys.value[0]})
              nextTick(()=>{
                Message.success({content:"删除成功",id:"del",duration:200})
                getmenudata();
              })
            } catch (err) {
  ...
  const handleDel=async(record:any)=>{
      try {
        Message.loading({content:"删除中",id:"del",duration:0})
        await del({id:record.id, tablename:handleTablename.value});
        fetchData();
