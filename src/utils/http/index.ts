...
const Main_url = import.meta.env.VITE_APP_ENV=="production"? window?.globalConfig.Main_url: window?.globalConfig.Main_url_dev;//前缀
...
const transform: AxiosTransform = {
  ...
  requestInterceptors: (config, options) => {
    ...
    if (token && (config as Recordable)?.requestOptions?.withToken !== false) {
      // const htoken =import.meta.env.VITE_OTTOKEN=='true'?Encrypt(token+"#"+timestamp):token;//可以缓存AES加密
      const htoken =import.meta.env.VITE_OTTOKEN=='true'?window.btoa(token+"#"+timestamp):token;//Base64 编解码(效率高)
      (config as Recordable).headers.Authorization = options.authenticationScheme ? `${options.authenticationScheme} ${htoken}`:htoken;
      (config as Recordable).headers["Ott"]=import.meta.env.VITE_OTTOKEN
    }
    //接口验证
    ...
    //设置语言
    (config as Recordable).headers["locale"]=localStorage.getItem('arco-locale');
    return config;
  },
